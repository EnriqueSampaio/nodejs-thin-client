...
class ClientSocket {
    ...
    async _processResponse(message) {
        ...
        while (this._buffer && this._offset < this._buffer.length) {
            if (this._buffer.length < this._offset + BinaryUtils.getSize(BinaryUtils.TYPE_CODE.INTEGER)) {
                break;
            }
            // Response length
            ...
            this._logMessage(requestId, false, this._buffer.getSlice(this._offset - length, length));
